...
// Assimp FBX aiMesh -> MeshSection 초기화
void MeshSection::InitializeFromAssimpMesh(ID3D11Device* device, const aiMesh* mesh)
{
    // [ 버텍스 데이터 추출 ]
...
// [ 버텍스 버퍼 생성 ]
void MeshSection::CreateVertexBuffer(ID3D11Device* device)
{
...
// [ 인덱스 버퍼 생성 ]
void MeshSection::CreateIndexBuffer(ID3D11Device* device)
{
    D3D11_BUFFER_DESC bd{};
    D3D11_SUBRESOURCE_DATA init{};
...
void MeshSection::Render(ID3D11DeviceContext* context, const Material& mat, ID3D11SamplerState* pSampler)
{
    // Vertex / Index 바인딩
